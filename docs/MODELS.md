...
**Summary:** Access to 119 AI models across OpenAI (85 models), Anthropic (8 models), and Google (26 models), including GPT-5.2, Claude Opus 4.5, Gemini 3 Pro, and various specialized models for chat, reasoning, audio, and image generation.
...
| Model ID | Type | Status | Modalities | Context Window |
|----------|------|--------|------------|----------------|
| `models/gemini-3-pro-preview` | chat | preview | text → text | 1M tokens |
| `models/gemini-3-flash-preview` | chat | preview | text → text | 1M tokens |
| `models/gemini-3-pro-image-preview` | chat | preview | text + image → text | 131K tokens |
| `models/gemini-2.5-flash` | chat | active | text → text | 1M tokens |
| `models/gemini-2.5-pro` | chat | active | text → text | 1M tokens |
| `models/gemini-2.5-flash-lite` | chat | active | text → text | 1M tokens |
| `models/gemini-2.5-flash-image` | chat | active | text + image → text | 32K tokens |
| `models/gemini-2.5-computer-use-preview-10-2025` | chat | preview | text → text | 131K tokens |
| `models/gemini-2.0-flash` | chat | active | text → text | 1M tokens |
| `models/gemini-2.0-flash-lite` | chat | active | text → text | 1M tokens |

## Model Types

- **chat**: General conversation and text completion
- **reasoning**: Advanced reasoning and problem-solving
- **audio**: Speech synthesis, transcription, and real-time audio
- **image**: Image generation and vision capabilities

## Status Legend

- **active**: Generally available for production use
- **preview**: Beta/experimental, subject to change